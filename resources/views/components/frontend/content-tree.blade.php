...
@foreach ($items as $index => $item)
    ...
    @if ($item->isFolder())
        {{-- ================= FOLDER (หัวข้อ) ================= --}}
        <div class="border-b border-red-100 bg-red-50/30">
            <div class="py-4 pr-4" style="{{ $indentStyle }}">
                <span class="font-bold text-red-900 text-base md:text-lg block">
    ...
    @else
        {{-- ================= FILE (รายการไฟล์) ================= --}}
        <div class="group bg-white border-b border-gray-100 last:border-0 hover:bg-red-50/20 transition-all duration-200 relative overflow-hidden">
            {{-- เส้นขอบแดงด้านซ้ายที่จะชัดขึ้นเวลาเอาเมาส์ชี้ (Hover Effect) --}}
            <div class="absolute left-0 top-0 bottom-0 w-1 bg-[#af1a00] opacity-0 group-hover:opacity-100 transition-opacity"></div>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between py-4 pr-4 gap-4" 
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-gray-900 text-sm md:text-base break-words mb-2.5">
                        <span class="font-bold text-black mr-1.5">{{ $currentNumber }}.</span>
                        <span class="font-medium group-hover:text-[#af1a00] transition-colors">{{ $item->name }}</span>
                    </div>

                    {{-- สถิติ: ออกแบบให้เป็นระเบียบ ไม่รกตา --}}
                    <div class="flex flex-wrap gap-3 text-[11px] md:text-xs text-gray-500">
                        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-gray-100 border border-gray-200">
                            <span class="w-1 h-1 rounded-full bg-gray-400"></span>
                            เข้าชม {{ number_format($item->view_count ?? 0) }} ครั้ง
                        </span>
                        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-red-50 text-red-700 border border-red-100">
                ...
                @if ($item->file_path)
                    <div class="flex items-center gap-2.5 pl-4 md:pl-0 shrink-0">
                        <a href="{{ route('file.view', $item->id) }}" target="_blank"
                ...
                @else
                    <span class="text-gray-400 text-xs italic pl-4 md:pl-0 font-light px-3 py-1 bg-gray-50 rounded">ไม่มีไฟล์แนบ</span>
                @endif
            </div>
        </div>
    @endif
