...
<!-- Desktop Navigation -->
<nav class="hidden lg:block bg-brand-red shadow-nav sticky top-0 z-50">
                        ...
                        @forelse($publishedYears as $year)
                            <li class="border-b border-white/5 last:border-0">
                                <a href="{{ route('scd.year', $year->year) }}"
                                    class="block py-3 px-5 text-white no-underline text-sm transition-all duration-200 hover:bg-brand-red-dark">
                                    SCD {{ $year->year }}
                                </a>
                            </li>
                        @empty
...
<header class="lg:hidden sticky w-full z-40 top-0" x-data="{ mobileMenuOpen: false, activeDropdown: null }">
    ...
    <nav class="bg-brand-red shadow-lg">
        <div class="flex flex-wrap justify-between items-center mx-auto px-4 py-2.5">
            ...
            <button @click="mobileMenuOpen = !mobileMenuOpen"
                class="text-white text-xl p-1.5 hover:bg-brand-red-dark rounded-lg transition-all duration-200">
    ...
    <nav x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-300"
        ...
        class="bg-brand-red-medium shadow-xl max-h-[calc(100vh-52px)] overflow-y-auto">
        <div class="px-3 py-3">
            <ul class="space-y-1">
                <!-- หน้าหลัก -->
                <li>
                    <a href="{{ route('home') }}"
                        class="block py-2.5 px-4 text-white text-sm font-medium transition-all duration-200 hover:bg-brand-red-dark rounded-lg">
                        หน้าหลัก
                    </a>
                </li>
                ...
                <!-- SCD Rankings -->
                <li>
                    <button @click="activeDropdown = activeDropdown === 'rankings' ? null : 'rankings'" type="button"
                        class="w-full flex justify-between items-center py-2.5 px-4 text-white text-sm font-medium text-left transition-all duration-200 hover:bg-brand-red-dark rounded-lg">
                        <span>SCD Rankings</span>
                        <i class="fa fa-chevron-down text-badge transition-transform duration-200"
                            :class="{ 'rotate-180': activeDropdown === 'rankings' }"></i>
                    </button>
                    <ul x-show="activeDropdown === 'rankings'" x-transition:enter="transition ease-out duration-200"
                        ...
                        class="bg-brand-red-dark rounded-lg mt-1 ml-3 overflow-hidden">
                        @forelse($publishedYears as $year)
                            <li>
                                <a href="{{ route('scd.year', $year->year) }}"
                                    class="block py-2 px-4 text-white text-sm transition-all duration-200 hover:bg-brand-red-darker">
                                    SCD {{ $year->year }}
                                </a>
                            </li>
                        @empty
                ...
                <!-- รายงานผล SCD -->
                <li>
                    <button @click="activeDropdown = activeDropdown === 'reports' ? null : 'reports'" type="button"
                        class="w-full flex justify-between items-center py-2.5 px-4 text-white text-sm font-medium text-left transition-all duration-200 hover:bg-brand-red-dark rounded-lg">
                        <span>รายงานผล SCD</span>
                        <i class="fa fa-chevron-down text-badge transition-transform duration-200"
                            :class="{ 'rotate-180': activeDropdown === 'reports' }"></i>
                    </button>
                    <ul x-show="activeDropdown === 'reports'" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0 max-h-0" x-transition:enter-end="opacity-100 max-h-96"
                        x-transition:leave="transition ease-in duration-150"
                        x-transition:leave-start="opacity-100 max-h-96" x-transition:leave-end="opacity-0 max-h-0"
                        class="bg-brand-red-dark rounded-lg mt-1 ml-3 overflow-hidden">
                ...
                <li>
                    <button @click="activeDropdown = activeDropdown === 'announcements' ? null : 'announcements'"
                        ...
                        <i class="fa fa-chevron-down text-badge transition-transform duration-200"
                            :class="{ 'rotate-180': activeDropdown === 'announcements' }"></i>
                    </button>
