<div>
    ...
    <div class="flex space-x-2">
        @if ($parentId === null)
            ...
            <button wire:click="openAddFolderModal"
        ...
        @else
            ...
            @if (!$hasFoldersInParent)
                <button wire:click="openAddFileModal"
    ...
    <x-backend.modal :show="$showModal">
        <x-backend.modal-form wire:submit="saveNode" :title="($editMode ? 'แก้ไข' : 'เพิ่ม') . ($type === 'folder' ? 'หมวดหมู่' : 'ไฟล์') . ($editMode ? '' : 'ใหม่')" :submitLabel="$editMode ? 'บันทึก' : 'เพิ่ม'">
            <!-- Sequence -->
            <div class="mb-4">
                <label for="sequence" class="block text-sm font-medium text-gray-700">ลำดับ <span
                        class="text-red-500">*</span></label>
                <input type="number" wire:model="sequence" id="sequence" min="1"
            ...
            @if ($type === 'folder' && $parentId === null)
                <div class="mb-4">
                    <label for="image" class="block text-sm font-medium text-gray-700">รูปภาพหมวดหมู่ <span
                            class="text-red-500">*</span></label>
                    ...
                    <p class="mt-1 text-xs text-gray-500">รองรับไฟล์รูปภาพ (JPG, PNG, สูงสุด 100MB)</p>
                    @error('image')
                        <span class="text-red-500 text-xs">{{ $message }}</span>
                    @enderror
            ...
            @if ($type === 'file')
                <div class="mb-4">
                    <label for="file" class="block text-sm font-medium text-gray-700">ไฟล์ PDF <span
                            class="text-red-500">*</span></label>
