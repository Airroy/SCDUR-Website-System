<div class="bg-white rounded-lg shadow">
    ...
    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        <h3 class="text-lg font-semibold text-gray-900">Slider Banner</h3>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="w-40 px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">รูปภาพ</th>
                        <th scope="col" class="hidden md:table-cell w-36 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ประเภท</th>
                        ...
                        <th scope="col" class="w-52 px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">จัดการ</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 sm:px-6 py-4 whitespace-nowrap">
                                @if($banner->image_path && Storage::disk('public')->exists($banner->image_path))
                                    <img src="{{ Storage::url($banner->image_path) }}" alt="Banner" class="h-12 w-20 object-cover rounded">
                                @else
                            ...
                            <td class="hidden lg:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                @if($banner->link_type === 'url' && $banner->link_url)
                                    <a href="{{ $banner->link_url }}" target="_blank" class="text-blue-600 hover:text-blue-800 truncate block max-w-xs">
                                        {{ Str::limit($banner->link_url, 40) }}
                                    </a>
                                @elseif($banner->link_type === 'pdf' && $banner->pdf_path)
                                    <a href="{{ Storage::url($banner->pdf_path) }}" target="_blank" class="text-orange-600 hover:text-orange-800">
