                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ชื่อรายงาน</th>
                                    <th scope="col" class="hidden md:table-cell w-40 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ไฟล์ PDF</th>
                                    <th scope="col" class="hidden lg:table-cell w-48 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">อัปเดต</th>
                                    <th scope="col" class="w-56 px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">จัดการ</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-3 sm:px-6 py-4">
                                            <div class="max-w-md overflow-hidden">
                                                <div class="text-sm font-medium text-gray-900 truncate w-full" title="{{ $report->file_name }}">{{ $report->file_name }}</div>
                                            </div>
                                        </td>
                                        <td class="hidden md:table-cell px-6 py-4">
                                            @if($report->file_path && Storage::disk('public')->exists($report->file_path))
                                                <a href="{{ Storage::url($report->file_path) }}" target="_blank" class="inline-flex items-center text-sm text-blue-600 hover:text-blue-800">
            ...
            <div class="px-6 py-4 space-y-4">
                ...
                <div>
                    <label for="file_name" class="block text-sm font-medium text-gray-700 mb-2">
                        ชื่อรายงาน <span class="text-red-500">*</span>
                    </label>
                    <input type="text" 
                           id="file_name"
                           wire:model="file_name" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                           placeholder="เช่น รายงานผล SCD ปีการศึกษา {{ $selectedYear->year }}">
                    @error('file_name') 
                ...
                <div>
                    ...
                    @if($editMode && $existingFile)
                        <div class="mb-3 p-3 bg-gray-50 rounded-lg flex items-center justify-between">
                            ...
                            <a href="{{ Storage::url($existingFile) }}" target="_blank" class="text-sm text-blue-600 hover:text-blue-800">
                                ดูไฟล์
                            </a>
                        </div>
                    @endif
